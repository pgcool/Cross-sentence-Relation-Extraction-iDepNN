...
class Node(object):
    def __init__(self, val=None, sent_idx = None):
        self.children = []
        self.val = val
        self.idx = None
        self.height = 1
...
def gen_nn_inputs(root_node, max_degree=None, only_leaves_have_vals=False,
    ...
    x, leaf_labels, leaf_computation_order = _get_leaf_vals(root_node)
    tree, internal_x, internal_labels, internal_computation_order = \
    ...
    if not only_leaves_have_vals:
        ...
        x.extend(internal_x)
        leaf_computation_order.extend(internal_computation_order)
    if max_degree is not None:
        assert all(len(t) == max_degree + 1 for t in tree), 'max_degree %s' %max_degree
    if with_labels:
...
def _get_leaf_vals(root_node):
    ...
    for idx, leaf in enumerate(reversed(all_leaves)):
        ...
        leaf_computation_order.append(leaf.sent_idx)
    return vals, labels, leaf_computation_order
...
def _get_tree_traversal(root_node, start_idx=0, max_degree=None):
    ...
    for layer in reversed(layers):
        for node in layer:
            ...
            internal_computation_order.append(node.sent_idx)
            idx += 1

    return tree, internal_vals, labels, internal_computation_order
...
class TreeRNN(object):
    ...
    def __init__(self, num_emb=None, emb_dim=None, hidden_dim=None, output_dim=None,
                 degree=2, learning_rate=0.01, momentum=0.9,
                 trainable_embeddings=False,
                 labels_on_nonroot_nodes=False,
                 irregular_tree=False, y_present = False):
        ...
        self.params = []
        self.recv_embeddings = theano.shared(self.init_matrix([self.num_emb, self.emb_dim]))
        ...
        if trainable_embeddings:
            self.params.append(self.recv_embeddings)
        ...
        emb_x = self.recv_embeddings[self.x]
        # T.neq(a,b) Returns a variable representing the result of logical inequality (a!=b).
        ...
        if y_present:
            ...
            self._train = theano.function(train_inputs,
                                          [self.loss, self.pred_y, self.final_state],
                                          updates=updates)
        ...
        recnn_inputs = [self.x, self.tree]
        self._get_final_state = theano.function(recnn_inputs,
                                                self.final_state)
        self._get_tree_states = theano.function(recnn_inputs,
                                                self.tree_states)
